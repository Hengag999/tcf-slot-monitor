...
async function main() {
  ...
  // A scrape failure already skips just that city (sites change; tolerated silently).
  // A notify/persist failure used to abort the whole run mid-loop — leaving later
  // cities unprocessed and the failing city's state unwritten, so it re-fired every
  // run. Instead, isolate each city: record the failure, finish the rest, then throw
  // at the end so genuine infra breakage (Discord 5xx, DB down) still alerts loudly.
  ...
  for (const city of cities) {
    ...
    try {
      await processCity(city, allSlots);
    } catch (err) {
      console.error(`[${city.key}] Notify/persist error, skipping:`, err);
      failedCities.push(city.key);
    }
  }

  if (failedCities.length > 0) {
    throw new Error(`Run completed with errors in: ${failedCities.join(", ")}`);
  }
}
